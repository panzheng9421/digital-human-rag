...
public class EnrichPrompt {
    ...
    private static final String SYSTEM_TEMPLATE = """
            ...
            [0s-{videoSeconds}s]主要事件：将{videoSeconds}秒均匀拆分为4-6个连续时间子段（每段约{videoSecondsDiv}秒），每个子段独立一行，格式为：
            [Xs-Ys]主要事件：<角色名>{动作+表情描写}{台词，用{}包裹}{散文内联节奏：重音落在『X』『Y』/语速变化/停顿约N秒}。
            每个子段有独立的动作+台词，动作有起点有终点，与台词融为一段连续叙事；最后一个子段的结尾以"表情自然定格/平滑过渡"收尾（不再单独写结束行）。
            ⚠️ 时间戳铁律：子段时间必须首尾相接、连续覆盖 [0s-{videoSeconds}s]，最后一个子段的结束值必须恰好等于{videoSeconds}，绝不允许出现超过{videoSeconds}s的时间戳，也不要输出单独的[{videoSeconds}s]结束行。如果台词太多放不下，宁可删减台词内容也不得延长时间。
    ...
    public String buildUser(String script, String personaName) {
        return """
                ...
                2. [事件脚本]将时长均匀拆分为4-6个连续时间子段（[Xs-Ys]主要事件：…），每个子段用散文内联重音/语速/停顿（如"重音落在『X』『Y』、语速放缓、停顿约1.5秒"），不得出现独立[台词节奏]表格，也不要单独的结束行
                3. [画面提示]内嵌的【音色锚定】为单行散文音色描述（声线+口音+音质+语速+语气）
                4. 【台词】段输出完整台词原文，与【事件脚本】中{}内台词完全一致，是[画面提示]的最后一段""".formatted(personaName, script);
    }
    ...
    /** 提取原始 [画面提示] 块（不做时间戳裁剪）。
     *  [画面提示] 为唯一输出块，依次包含八段（…→保持一致→台词）+ 内嵌【音色锚定】，
     *  其中【台词】是末段、属于提示词正文，因此截取到字符串末尾（不再在 [台词] 处截断）。 */
    public static String extractVisualRaw(String enriched) {
        ...
        if (start < 0) start = 0;
        return enriched.substring(start).trim();
    }
